...
export default function ShutdownEdit({
    shutdown,
    locations,
    machines,
    shutdown_types,
    ...
    const { data, setData, put, processing, errors } = useForm({
        ...
        machine_id: shutdown.machine_id?.toString() || '',
        location_id: shutdown.location_id.toString(),
        start_at: shutdown.start_at.slice(0, 16),
        end_at: shutdown.end_at.slice(0, 16),
        shutdown_type: shutdown.shutdown_type,
    });

    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        put(`/planning/shutdowns/${shutdown.id}`);
    };
    ...
    return (
        <AppLayout>
            <Head title={t('planning.shutdowns.edit', 'Edit Shutdown')} />
            ...
            <div className="container mx-auto max-w-2xl space-y-6 py-6">
                ...
                <div>
                    <Button
                        variant="ghost"
                        size="sm"
                        onClick={() => router.visit(`/planning/shutdowns/${shutdown.id}`)}
                        className="mb-2 text-muted-foreground"
                    >
                        <ArrowLeft className="mr-2 h-4 w-4" />
                        {t('planning.shutdowns.back_to_details', 'Back to Details')}
                    </Button>
                    <h1 className="text-3xl font-bold tracking-tight">
                        {t('planning.shutdowns.edit', 'Edit Shutdown')}
                    </h1>
                    <p className="text-muted-foreground">
                        {t('planning.shutdowns.edit_description', 'Update the shutdown details')}
                    </p>
                </div>
